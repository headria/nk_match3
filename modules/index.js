...
(function (Bucket) {
    ...
    function InitBucket(nk) {
        ...
        var InitBucketKey = "".concat(Bucket.configs.weekly.tournamentID, "#").concat(initValue);
...
var WeeklyGetRecordsRPC = function (ctx, logger, nk) {
    ...
    //get user bucket
    var userBucket = nk.storageRead([{ collection: collection, key: key, userId: userId }]);
    ...
    while (true) {
        try {
            var _a = getLatestBucketId(nk), latestId = _a.latestId, latestVersion = _a.latestVersion;
            var _b = getBucketById(nk, leaderBoadrdId, latestId), bucket = _b.bucket, version = _b.version;
            ...
            if (bucket.userIds.length >= bucketSize) {
                ...
                latestId = latestId + 1;
                version = createNewBucket(nk, leaderBoadrdId, latestId, latestVersion, bucket);
            ...
            //return records
            var tournament = nk.tournamentRecordsList(Bucket.configs.weekly.tournamentID, bucket.userIds);
